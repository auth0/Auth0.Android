...
class CryptoUtil {
    ...
    KeyStore.PrivateKeyEntry getRSAKeyEntry() throws CryptoException, IncompatibleDeviceException {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                 ...
                 * digest. Our OAEP_SPEC uses MGF1/SHA-1. The setMgf1Digests API only exists on
                 * API 35+ (VANILLA_ICE_CREAM); below that KeyMint defaults the MGF1 digest
                 * (historically to SHA-1), so leaving it unset worked. On newer Keystore2 hardware
                 * the authorized MGF1 digest set is strictly enforced and its default is
                 * vendor-dependent (some default to SHA-256 only), which rejects our MGF1/SHA-1
                 * cipher with INCOMPATIBLE_MGF_DIGEST ("Incompatible padding mode"). Where the API is
                 * available, authorize SHA-1 (used by the current cipher) and SHA-256 explicitly so
                 * the key no longer depends on the vendor default and remains compatible if the
                 * cipher moves to MGF1/SHA-256 in the future. See OAEP_SPEC.
                 */
    ...
    byte[] RSADecrypt(byte[] encryptedInput) throws IncompatibleDeviceException, CryptoException {
        try {
             ...
             *      Keystore2 hardware (API 35+) an existing RSA key whose authorized MGF1 digest set
